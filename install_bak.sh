...
check_env()
{
  ...
  # check_value
  # 检查配置项是否为空，为空给出提示是否不安装此模块，输入y后取消安装此模块，继续安装其他模块，n则退出安装程序
  show_info " checking ./conf/ns.ini now "
  if [ ! -n "$hm_ip" ]
    then
      read -r -p "<hm> is null in ./conf/ns.ini file, will not install top_stat_agent:  [y/n]" input
      case $input in
        [Yy][Ee][Ss]|[Yy])
          ;;
        *)
          show_err " install failed, exit"
          exit 1
          ;;
      esac
  else
  ...
  if [ ! -n "$es_ip" ]
    then
      read -r -p "<es_ip> is null in ./conf/ns.ini file, will not install es_stat_agent:  [y/n]" input
      case $input in
        [Yy][Ee][Ss]|[Yy])
          ;;
        *)
  ...
  if [ ! -n "$interds_ip" ]
    then
      read -r -p "<RTPMaster> is null in ./conf/ns.ini file, will not install interds_stat_agent:  [y/n]" input
      case $input in
        [Yy][Ee][Ss]|[Yy])
          ;;
        *)
  ...
  do
    ...
    then
      ...
      show_info " connect $node no passwd is successful."
    fi
  done

  # echo "all_ip:" ${all_ip[*]}
  # echo "all_app:" ${all_app[*]}
  # show_info " install_app :  ${all_app[*]}"
}
...
install_interds_stat_agent()
{
  ...
  ssh $interds_ip "source /etc/profile; rm -rf $NebulaSolarDash_PATH; mkdir -p $NebulaSolarDash_PATH"
  ssh $interds_ip "rm -rf /etc/cron.d/daemon_start_interds_agent"
  scp -r * root@$interds_ip:$NebulaSolarDash_PATH 1>/dev/null 2>/dev/null
  scp $NebulaSolarDash_PATH/lib/common_lib_nooracle.py root@$interds_ip:$NebulaSolarDash_PATH/lib/common_lib.py 1>/dev/null 2>/dev/null
  ssh $interds_ip "source /etc/profile; cp -r $NebulaSolarDash_PATH/crond/daemon_start_interds_agent /etc/cron.d/ && service crond restart 1>/dev/null 2>/dev/null"
  show_info " install interds_stat_agent successful."
}

install_cx_Oracle()
{
  cd $NebulaSolarDash_PATH/lib/ && tar xzvf cx_Oracle-5.1.2.tar.gz && cd cx_Oracle-5.1.2/ && /usr/bin/python setup.py install
}
